...
//	}
    ...
    {        
        ...
        cookie.setPath("/");		//cookie对根目录地下的所有路径都可以访问cookie
        if (maxAge>0) 
        	cookie.setMaxAge(maxAge);	//只要大于0那么就设置相应的值
    ...
    {
    	Map<String, Cookie> cookieMap = WebUtil.readCookieMap(request);	//取得所有的cookie
        if(cookieMap.containsKey(name))
        {
            Cookie cookie = (Cookie)cookieMap.get(name);	//根据cookie的名字取得相应的cookie
            return cookie.getValue(); 	//返回相应的cookie的值
        }
        else
        {
            return null;
    ...
    /**
     * 这个函数是为了把所有的cookie取出来放到一个map里面去
     * @param request
     * @return
     */
    protected static Map<String, Cookie> readCookieMap(HttpServletRequest request) 
    {
        ...
        //从request里面取得所有的cookie
        Cookie[] cookies = request.getCookies();
        if (null != cookies) 
        {
            for (int i = 0; i < cookies.length; ++i) 
            {
            	//吧所有的cookie存放到map里面去，用cookie的名字作为key
